...
<body>
    ...
    <?php
        ...
        function realizarReserva($nombre, $personas, $exterior = false, $hora = "20:00") {
            // TODO: Las mesas disponibles no se actualizan bien
            global $arrayReservas;
            global $mesasDisponibles;
            ...
            if ($mesasNecesarias > $mesasDisponibles) { // Si no hay mesas suficientes
                ...
                $mesasDisponibles -= $mesasNecesarias;
                return true;
            }
        }
        ...
        /**
         * Valida si la hora pasada por parámetro es válida (está entre las 20:00 y las 22:00 y cumple el formato)
         */
        function validarHora($hora) {
            ...
            if (preg_match($formato, $hora)) { // Si la hora cumple con el formato
                $arrayHora = explode(":", $hora); // Dividir las horas y los minutos
                $horas = intval($arrayHora[0]);
                $minutos = intval($arrayHora[1]);
                // Comprobar que las horas esten entre 20 y 22
                if ($horas > $horaMax || $horas < $horaMin) { // Si la hora sobrepasa algún límite
                    return false;
                } else { // Si el formato es correcto y además la hora se encuentra entre los límites
                    if ($horas == $horaMax && $minutos != $minMin) { // Si la hora es 22 no puede llevar minutos
                        return false;
                    } 
                    return true;
                }
                    ...
                    case "reservar":
                        ...
                        if (
                            ...
                            if (isset($_GET["hora"]) && !empty($_GET["hora"]) && !validarHora($_GET["hora"])) { // Si se ha introducido una hora pero NO es correcta
                                ...
                                $nombre = isset($_GET["nombre"]) ? htmlspecialchars($_GET["nombre"]) : "";
                                $personas = isset($_GET["personas"]) ? intval(htmlspecialchars($_GET["personas"])) : "";
